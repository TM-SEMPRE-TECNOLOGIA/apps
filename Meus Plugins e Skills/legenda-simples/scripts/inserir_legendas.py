...
def coletar_operacoes(body, children):
    ...
    """
    ops = []
    dados_dependencia_encontrado = False

    for i, child in enumerate(children):
        tag = child.tag.split('}')[-1] if '}' in child.tag else child.tag

        if tag == 'p' and "Dados da Dependência" in get_text(child):
            dados_dependencia_encontrado = True

        if not dados_dependencia_encontrado:
            continue
