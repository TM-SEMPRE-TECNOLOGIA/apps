...
def main(docx_path, out_path):
    ...
    for occ in occurrences:
        ...
        all_items.append({
            'code': code,
            'desc': occ['desc'].split('(')[0].strip() if occ['desc'] else '',
            'qty_str': fmt(occ['total_value']),
            'unit': occ['unit'] if occ['unit'] else occ['htype']
        })
    ...
    for code, d in consolidated.items():
        consol_list.append({
            'code': code,
            'desc': d['desc'].split('(')[0].strip() if d['desc'] else '',
            'qty_str': fmt(d['total']),
            'unit': d['unit'] if d['unit'] else d['htype']
        })
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print('Uso: python3 extrair_itens_v2.py <input.docx> <output.xlsx>')
        sys.exit(1)
