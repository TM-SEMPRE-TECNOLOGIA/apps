...
## V4.1 (Maio 2026) — Pivô Industrial & NOVO LARANJADO

### Objetivo
Consolidar a arquitetura da V4 com um Design System sênior de engenharia civil, eliminando a estética SaaS e focando em performance e automação de inicialização.

### Novas Features (V4.1)
- **Launcher Unificado:** Criação do `START_V4.1.bat` para automação completa de setup.
- **Design System v3 (NOVO LARANJADO):** Implementação da paleta construction orange, hairlines de 1px e tipografia Roboto Slab.
- **Drawer Terminal:** Refatoração do `ConsoleWatcher` para o rodapé (gaveta) com microinteração de push-up.
- **Preview Panel 2.0:** Painel lateral de visualização do documento totalmente reativo e com seletores de contrato/modelo integrados.
- **Arquitetura Limpa:** Purga de arquivos legados na raiz e centralização de docs vivos na pasta `.Atualiza`.
